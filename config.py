...
class Settings(BaseSettings):
    ...
    database_query_timeout: int = Field(
        default=30, validation_alias="DATABASE_QUERY_TIMEOUT"  # 30 seconds
    )
    ...
    cors_origins: str = Field(
        default="", validation_alias="CORS_ORIGINS"  # Empty = no CORS (secure default)
    )
    cors_allow_credentials: bool = Field(
        default=False, validation_alias="CORS_ALLOW_CREDENTIALS"  # Secure default
    )
    cors_allow_methods: str = Field(
        default="GET,POST,PUT,DELETE", validation_alias="CORS_ALLOW_METHODS"
    )
    cors_allow_headers: str = Field(
        default="Authorization,Content-Type", validation_alias="CORS_ALLOW_HEADERS"
    )
    cors_max_age: int = Field(default=600, validation_alias="CORS_MAX_AGE")  # 10 minutes
    ...
    pepper_totp_secret_key: Optional[str] = Field(
        default=None, validation_alias="PEPPER_TOTP_SECRET_KEY"
    )
    ...
    pepper_mtls_cert: str = Field(
        default="/certs/pepper-server.crt", validation_alias="PEPPER_MTLS_CERT"
    )
    pepper_mtls_key: str = Field(
        default="/certs/pepper-server.key", validation_alias="PEPPER_MTLS_KEY"
    )
    pepper_mtls_client_ca: str = Field(
        default="/certs/client-ca.crt", validation_alias="PEPPER_MTLS_CLIENT_CA"
    )
    ...
    integrity_mtls_cert: str = Field(
        default="/certs/integrity-server.crt", validation_alias="INTEGRITY_MTLS_CERT"
    )
    integrity_mtls_key: str = Field(
        default="/certs/integrity-server.key", validation_alias="INTEGRITY_MTLS_KEY"
    )
    integrity_mtls_client_ca: str = Field(
...
def validate_config(settings: Settings):
    ...
    # CRIT-10: Reject hardcoded/default database credentials
    _insecure_markers = {
        "change-me",
        "changeme",
        "change_me",
    ...
    if settings.pepper_enabled:
        ...
        if len(settings.pepper_totp_secret_key) != 44:
            raise ValueError("PEPPER_TOTP_SECRET_KEY must be a valid Fernet key (44 characters)")
